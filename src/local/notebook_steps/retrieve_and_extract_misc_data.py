...
def configure_notebooks(
    ...
    configured_notebooks = [
        ...
        ConfiguredNotebook(
            unconfigured_notebook=uc_nbs_dict[Path("000y_retrieve-misc-data") / "0002_primap"],
            configuration=(config_step.primap.download_url,),
            dependencies=(),
            targets=(config_step.primap.raw_dir / config_step.primap.download_url.url.split("/")[-1],),
            config_file=config_bundle.config_hydrated_path,
            step_config_id=step_config_id,
        ),
        ConfiguredNotebook(
            unconfigured_notebook=uc_nbs_dict[Path("000y_retrieve-misc-data") / "0003_hadCRUT5"],
            configuration=(config_step.hadcrut5.download_url,),
            dependencies=(),
            targets=(config_step.hadcrut5.raw_dir / config_step.hadcrut5.download_url.url.split("/")[-1],),
            config_file=config_bundle.config_hydrated_path,
            step_config_id=step_config_id,
        ),
    ]
