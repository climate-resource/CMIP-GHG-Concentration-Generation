...
# %%
allyears_latitudinal_gradient = allyears_pcs @ lat_grad_eofs_obs_network["eofs"]
allyears_latitudinal_gradient

# %% [markdown]
# This should exactly match our observational network latitudinal gradient in the years they overlap.

# %%
xr.testing.assert_allclose(
    allyears_latitudinal_gradient.sel(year=lat_grad_eofs_obs_network["year"]),
...
# %% [markdown]
# ### Interpolate to a year-month field

# %% [markdown]
# #### Global-mean

# %%
allyears_global_annual_mean_monthly = (
    local.mean_preserving_interpolation.interpolate_annual_mean_to_monthly(
        allyears_global_annual_mean
    )
)
local.xarray_time.convert_year_month_to_time(allyears_global_annual_mean_monthly).plot()
...
# %%
allyears_pcs_monthly = allyears_pcs.groupby("eof", squeeze=False).apply(
    local.mean_preserving_interpolation.interpolate_annual_mean_to_monthly
)
allyears_pcs_monthly

# %%
allyears_latitudinal_gradient_monthly = (
    allyears_pcs_monthly @ lat_grad_eofs_obs_network["eofs"]
)
allyears_latitudinal_gradient_monthly

# %% [markdown]
# Check that annual mean of interpolated gradient is unchanged.

# %%
xr.testing.assert_allclose(
    allyears_latitudinal_gradient_monthly.mean("month"),
    allyears_latitudinal_gradient,
)

# %% [markdown]
# Check that the annual-mean of the field is unchanged.

# %%
xr.testing.assert_allclose(
    (allyears_latitudinal_gradient_monthly + allyears_global_annual_mean_monthly).mean(
        "month"
    ),
    allyears_full_field,
)
...
allyears_latitudinal_gradient_monthly.pint.dequantify().to_netcdf(
    config_step.latitudinal_gradient_file
)
allyears_latitudinal_gradient_monthly

# %%
config_step.global_annual_mean_monthly_file.parent.mkdir(exist_ok=True, parents=True)
allyears_global_annual_mean_monthly.pint.dequantify().to_netcdf(
