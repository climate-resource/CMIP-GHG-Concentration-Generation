...
# %%
# time_demo = monthly_15_degree.mean("month").sel(lat=7.5).squeeze()
# time_demo_monthly = local.mean_preserving_interpolation.interpolate_annual_mean_to_monthly(
#     time_demo
# )
...
# local.xarray_time.convert_year_to_time(time_demo).plot.scatter(x="time", color="tab:orange", zorder=3, alpha=0.5, ax=axes[0])

# local.xarray_time.convert_year_month_to_time(time_demo_monthly.sel(year=range(2010, 2023))).plot(ax=axes[1])
# local.xarray_time.convert_year_to_time(time_demo.sel(year=range(2010, 2023))).plot.scatter(x="time", color="tab:orange", zorder=3, alpha=0.5, ax=axes[1])

# local.xarray_time.convert_year_month_to_time(time_demo_monthly.sel(year=range(1, 10))).plot(ax=axes[2])
# local.xarray_time.convert_year_to_time(time_demo.sel(year=range(1, 10))).plot.scatter(x="time", color="tab:orange", zorder=3, alpha=0.5, ax=axes[2])
...
# %%
nh_mean = local.xarray_space.calculate_global_mean_from_lon_mean(
    monthly_15_degree.sel(lat=monthly_15_degree["lat"] > 0)
)
nh_mean

# %%
sh_mean = local.xarray_space.calculate_global_mean_from_lon_mean(
    monthly_15_degree.sel(lat=monthly_15_degree["lat"] < 0)
)
sh_mean
